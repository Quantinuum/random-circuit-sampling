...
def load_logXEB_results(Nd_pairs_xeb, n_resamples):
    probabilities = dict()
    logxeb_counts = dict()
    logxeb = dict()
    logxeb_uncertainties = dict()
    ...
    for i, Nd in enumerate(Nd_pairs_xeb):
        ...
        if N <= 40:
            ...
            for r in range(1,51):
                ...
                counts_dict = {ast.literal_eval(k):v for k,v in counts_json.items()}
                prob_dict = {k:abs(v)**2 for k,v in amp_dict.items()}
                logxeb[(N,d,r)]= logXEB(counts_dict, prob_dict, N)
                probabilities[(N,d)].append(prob_dict)
                logxeb_counts[(N,d)].append(counts_dict)
            logxeb[(N,d)] = np.mean([logxeb[(N,d,r)] for r in range(1,51)])
            logxeb_uncertainties[(N,d)] = logXEB_bootstrap_uncertainty(logxeb[(N,d)],logxeb_counts[(N,d)],probabilities[(N,d)],n_resamples)
    return logxeb, logxeb_uncertainties
...
def load_logXEB_results_N40_verification(Nd_pairs_xeb_N40_verification, n_resamples):
    probabilities_N40_verification = dict()
    logxeb_counts_N40_verification = dict()
    logxeb_N40_verification = dict()
    logxeb_uncertainties_N40_verification = dict()
    for i, Nd in enumerate(Nd_pairs_xeb_N40_verification):
        N = Nd[0]
        d = Nd[1]
        probabilities_N40_verification[(N,d)] = []
        logxeb_counts_N40_verification[(N,d)] = []
        for r in range(1,51):
            filename=f'amplitudes/N40_verification/N{N}_d{d}_XEB/N{N}_d{d}_r{r}_XEB_amplitudes.json'
            amps_json = load_data(filename)
            filename = f'results/N40_verification/N{N}_d{d}_XEB/N{N}_d{d}_r{r}_XEB_counts.json'
            ...
            counts_dict = {ast.literal_eval(k):v for k,v in counts_json.items()}
            prob_dict = {k:abs(v)**2 for k,v in amp_dict.items()}
            logxeb_N40_verification[(N,d,r)] = logXEB(counts_dict, prob_dict, N)
            probabilities_N40_verification[(N,d)].append(prob_dict)
            logxeb_counts_N40_verification[(N,d)].append(counts_dict)
        logxeb_N40_verification[(N,d)] = np.mean([logxeb_N40_verification[(N,d,r)] for r in range(1,51)])
        logxeb_uncertainties_N40_verification[(N,d)] = logXEB_bootstrap_uncertainty(logxeb_N40_verification[(N,d)],logxeb_counts_N40_verification[(N,d)],probabilities_N40_verification[(N,d)],n_resamples)
    return logxeb_N40_verification, logxeb_uncertainties_N40_verification
