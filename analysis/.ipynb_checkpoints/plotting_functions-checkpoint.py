...
def plot_t1qrb_survival_curve_N40_verification(t1qrb_survival_N40_verification, t1qrb_uncertainties_N40_verification, t1qrb_seq_lengths, t1qrb_seq_lengths_interp):
    ...
    plt.ylim([0.94, 1.002])
    plt.legend()
    plt.savefig('N40Verification_memory.pdf',format='pdf')
    plt.show()
...
def plot_depth12_data(xeb, xeb_uncertainties, logxeb, logxeb_uncertainties, mb_fidelity, mb_uncertainties, gc_depth12, gc_depth12_lower, gc_depth12_upper, Nrange, Nrange_interp):
    fig, ax = plt.subplots()
    fig.set_facecolor('white')
    ...
    ax.errorbar(Nrange[:4],[xeb[(N,12)] for N in Nrange[:4]], yerr = np.array([xeb_uncertainties[(N,12)] for N in Nrange[:4]]).transpose() ,fmt="o",color=color_list[1], markerfacecolor=[1,1,1],markeredgecolor=color_list[1],label = "$F_{XEB}^{lin}$ (linear cross-entropy)")
    ax.errorbar(Nrange[:4],[logxeb[(N,12)] for N in Nrange[:4]], yerr = np.array([logxeb_uncertainties[(N,12)] for N in Nrange[:4]]).transpose() ,fmt="o",color=color_list[2], markerfacecolor=[1,1,1],markeredgecolor=color_list[2],label = "$F_{XEB}^{log}$ (log cross-entropy)")
    ...
    plt.title('d = 12')
    plt.ylim([0.01, 0.9])
    plt.savefig('Nscan_Dec_9.pdf',bbox_inches='tight',format='pdf')
    plt.show()
...
def plot_N56_data(mb_fidelity, mb_uncertainties, gc_N56, gc_N56_lower, gc_N56_upper, spam_fid, spam_uncert, drange, drange_interp):
    ...
    plt.savefig('N56_dscan_Dec_9.pdf',bbox_inches='tight',format='pdf')
    plt.show()

def plot_N40_data(xeb_N40_verification, xeb_uncertainties_N40_verification, logxeb_N40_verification, logxeb_uncertainties_N40_verification, mb_fidelity_N40_verification, mb_uncertainties_N40_verification, gc_N40, gc_N40_lower, gc_N40_upper, drange, drange_interp):
    fig, ax = plt.subplots()
    fig.set_facecolor('white')
    color_list = [plt.get_cmap("Dark2").colors[i] for i in range(7)]
    ...
    ax.errorbar(drange,[logxeb_N40_verification[(40,d)] for d in drange], yerr = np.array([logxeb_uncertainties_N40_verification[(40,d)] for d in drange]).transpose(),fmt="o",color=color_list[2], markerfacecolor=[1,1,1],markeredgecolor=color_list[2],label = "$F_{XEB}^{log}$ (log cross-entropy)")
    ...
    #ax.set_yticks([])
    plt.tick_params(axis='y', which='both', left=False, labelleft=False)
    plt.savefig('N40_dscan_Dec_9.pdf',bbox_inches='tight',format='pdf')
    plt.show()