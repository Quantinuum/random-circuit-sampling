...
def logXEB(exp_counts, ideal_probs, N):
        shots = sum(exp_counts.values())
        avg = 0
        for bs in list(exp_counts.keys()):
            avg += exp_counts[bs] * np.log(ideal_probs[bs])
        avg /= shots

        return avg + np.euler_gamma + N*np.log(2)
...
def logXEB_bootstrap_uncertainty(logxeb_emp, counts, probs, n_resamples):
    bs_with_corresp_probs = []
    for i, counter in enumerate(counts):
        for bs in counter.keys():
            for _ in range(counter[bs]):
    ...
    for _ in range(n_resamples):
        ...
        avg = 0
        for bs_prob in bootstrapped_shots_with_probs:
            avg += np.log(bs_prob[1])
        avg /= num_total_shots
        
        logxeb = avg + np.euler_gamma + N*np.log(2)
        logxeb_list.append(logxeb)

    lower = 2*logxeb_emp - np.quantile(logxeb_list,1-0.1587)
    upper = 2*logxeb_emp - np.quantile(logxeb_list, 0.1587)
    return logxeb_emp - lower, upper - logxeb_emp
